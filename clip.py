...
import numpy as np
import math
from pseudorandom import ReservoirSample, getGause

from perturb import slicing
...
def noise(dy_dx: list,
    ...
    layer_dims_pool, \
        layer_params_num_pool, \
            layer_params_num_gcd, \
                slice_indices, \
                    slice_params_indice = slicing(dy_dx[0], sensitivity, slices_num)
    ...
    for layer_index in range(len(layer_dims_pool)):
        if layer_index == 0:
            ...
            params_end_indice = params_start_indice + layer_params_num_pool[layer_index] - 1 
